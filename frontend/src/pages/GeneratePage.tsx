...
import { Sparkles, Sliders, CheckCircle2, Loader2, ArrowLeft } from 'lucide-react';
...
const GeneratePage: React.FC = () => {
  ...
  const validateUrl = (value: string) => {
    ...
    if (!id) {
      setValidationError('Invalid YouTube URL. Please use standard formats (e.g. youtube.com/watch?v=XXXX or youtu.be/XXXX)');
      return false;
    }
  ...
  const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUrl(val);
    if (validationError) {
      // Clear or update error on change if they've already erred
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Once loader steps complete (or shortly after API returns), navigate to viewer
      setTimeout(() => {
        navigate(`/synopsis/${result.id}`);
      }, 7200); // Give loader simulation time to run through nicely (total ~7s)
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <span className="text-[10px] text-zinc-500 block leading-tight">
                      Short (Executive Bulletins), Medium (Chapter Logs), Long (Complete breakdowns).
                    </span>
                  </div>

                  {/* Option 2: NLP Toggles */}
                  <div className="space-y-4">
                    <span className="text-xs font-bold uppercase tracking-wider text-zinc-400 block">
                      Enable Advanced NLP Filters
                    </span>
